...
//This page deletes a person as band contact & adds a member to an existing band
...
include("../includes/functions.inc");
  ...
  //delete person as band contact - set contact_person=0
  if (!isset($_POST['add_band_member']))
  {
   ...
   //update row in band member table
   $change_contact_person = "UPDATE band_member SET contact_person=0 WHERE band_id = '$band_id' AND person_id = '$person_id'";
    ...
    if (mysql_num_rows($get_info_res)> 0)
    {
        ...
        <option value=\"\">--Select a person--</option>";
         ...
         <input type=\"submit\" name=\"add_band_member\" value=\"Add a band member\"></form></td></tr></table></div>";
            ...
            //look for band members
            $get_people = "SELECT person.id, fname, lname, phone, contact_person FROM person, band, band_member WHERE band_member.person_id = person.id AND person.is_active = 1 AND band.id = band_member.band_id AND band_id = '$id' ORDER BY lname, fname ASC";
            $get_people_res = mysql_query($get_people);
