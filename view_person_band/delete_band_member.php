...
//This page deletes a band member & adds a member to an existing band
...
//delete band member
 if (!isset($_POST['add_band_member']))
 {
    ...
    //delete row from band member table
    $delete_band_member = "DELETE FROM band_member WHERE band_id = $band_id AND person_id = $person_id";
    $delete_band_member_res = mysql_query($delete_band_member) or die ("ERROR: " . mysql_errno() . "-" . mysql_error() );
    ...
    if (mysql_num_rows($get_info_res)> 0)
    {
        ...
        <option value=\"\">--Select a person--</option>";
         ...
         <input type=\"submit\" name=\"add_band_member\" value=\"Add a band member\"></form></td></tr></table></div>";
            ...
            <td align=\"right\"><a href=\"delete_band_contact.php?person_id=$person_id&band_id=$id\" onclick=\"return confirm('Are you sure you want to remove this person as band contact?')\">Remove person a band contact</a> | <a href=\"person_record.php?person_id=$person_id\">View person record</a> | <a href=\"delete_band_member.php?person_id=$person_id&band_id=$id\" onclick=\"return confirm('Are you sure you want to delete this band member?')\">Delete band member</a></td></tr>";
