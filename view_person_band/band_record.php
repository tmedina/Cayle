...
//This page shows the band record from the search box and a person record

//Checks to see if user is logged in; if not redirect to log-in page & adds a member to an existing band
session_start();
//print_r ($_SESSION);
...
if((!isset($_SESSION["logged"])) && !$_SESSION["logged"])
{
    ...
    //connect to the db & functions.inc file
       include("../includes/dbconnect.inc");
       include("../includes/functions.inc");

// view a band record from person record
     ...
     } else
     {
        ...
        <select name=\"sel_id\">
         ...
         <input type=\"submit\" name=\"add_band_member\" value=\"Add a band member\"></form></td></tr></table></div>";
            ...
            //look for band members
            $get_people = "SELECT person.id, fname, lname, phone, contact_person FROM person, band, band_member WHERE band_member.person_id = person.id AND band.id = band_member.band_id AND person.is_active = 1 AND band_id = '$id' ORDER BY lname, fname ASC";
            $get_people_res = mysql_query($get_people) or die ("ERROR: " . mysql_errno() . "-" . mysql_error() );
...
<html>
    ...
    <!--
    Form validation - makes sure a value is selected from the drop down menu
    -->
