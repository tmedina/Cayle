...
$display_block .= "<h2 align=\"left\">View room rental charges</h2>";
...
$get_rev = "SELECT reservation_entry_id, start_time, fname, lname, room_name,
reservation_status, reservation_transaction.amount
FROM reservation_transaction, reservation_entry, person, reservation_room
...
AND reservation_entry.start_time >= $utf_start_date  AND reservation_entry.end_time <= $utf_end_date
ORDER BY reservation_entry_id;";
$get_rev_res = mysql_query($get_rev) or die ("ERROR 1: " . mysql_errno() . "-" . mysql_error() );

if (mysql_num_rows($get_rev_res) == 0)
{
    $display_block .= "<h2 align=\"left\">Room rental charges for reservations with start times between<br />&nbsp;&nbsp; " . $req_start_date . " to " . $req_end_date    . "</h2>";
    $display_block .= "<div id=\"links_report_new\"><a href=\"../admin/index.php\">Back to reports index</a> | <a href=\"room_rental_charges.php\">Select a different date range</a></div>";
    $display_block .= "<div id=\"text\" align=\"left\"><p>No room rental charges available during the requested date range</p></div>";
}
...
elseif (mysql_num_rows($get_rev_res) > 0)

    {
        ...
        $display_block .= "<th>Reservation id</th><th>Reservation start time</th><th>Person name</th><th>Room</th><th>Reservation status</th><th>Amount charged</th>";
        ...
        while ($rev_info = mysql_fetch_array($get_rev_res))
            ...
            $display_block .= "<tr><td>" . $res_id . "</td><td>" . utf_period_2_date($start_time) . "</td><td>" . $fname . " " . $lname . "</td><td>" . $room_name . "</td><td>" . $reservation_status . "</td><td>" . number_format(abs($amount), 2) . "</td></tr>";
            
            }
            $display_block .= "<tr><td colspan=\"5\"><b>Total charges for this time period</td><td><b>" . number_format(abs($grand_total_charges), 2) . "</b></td></tr>";
...
$sql = "SELECT reservation_entry_id, start_time, fname, lname, room_name,
reservation_status, reservation_transaction.amount
FROM reservation_transaction, reservation_entry, person, reservation_room
WHERE reservation_entry.id = reservation_transaction.reservation_entry_id
AND reservation_entry.person_id = person.id
...
<html>
 <head>
    <title>Report: room rental charges</title>
