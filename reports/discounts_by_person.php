...
if (!isset($_POST['run_report']))
{
    // view the drop down list of active person record names & phone numbers available
     ...
     if (mysql_num_rows($get_list_res) <1)
     {
         ...
         $display_block .= "<h2 align=\"left\">Discounts by person</h2>";
...
{           include("../includes/dbconnect.inc");
            ...
            $get_discounts = "SELECT reservation_entry.id, band_name, start_time, misc_charge.name, reservation_transaction.comment,
            ...
            ORDER BY start_time, misc_charge.name;";
            $get_discounts_res = mysql_query($get_discounts) or die ("ERROR 3: " . mysql_errno() . "-" . mysql_error() );


            //if bands exist display them
            
           if (mysql_num_rows($get_discounts_res)> 0)
            {
                $display_block .= "<h2 align=\"left\">Discounts for " . $fname . " " . $lname .  " from " . $req_start_date . " to " . $req_end_date    . "</h2>";
                $display_block .= "<div id=\"links_report_new\"><a href=\"../admin/index.php\">Back to reports index</a> | <a href=\"discounts_by_person.php\">Select a different person</a></div>";
                $display_block .= "<form method=\"post\" action=\"$_SERVER[PHP_SELF]\">";
                $display_block .= "<div id=\"text\" align=\"left\"><table border=\"1\" cellspacing=\"5\" cellpadding=\"5\">";
                $display_block .= "<th>Reservation id</th><th>Band</th><th>Start time</th><th>Discount name</th><th>Comment</th><th>Discount amount</th><th>Discount date/time</th>";
            ...
            $display_block .= "<tr><td>" . $id . "</td><td>" . $band_name . "</td><td>" . utf_period_2_date($start_time) . "</td><td>" . $name . "</td><td>" . $comment . "</td><td>" . $amount . "</td><td>" . $updated_at . "</td></tr>";
           
            }
            $display_block .= "<tr><td colspan=\"5\"><b>Total discount amount for this time period</td><td colspan=\"2\">" . number_format($total_discounted, 2) . "</td></tr>";
...

            {
                $display_block .= "<h2 align=\"left\">Discounts for " . $fname . " " . $lname .  " from " . $req_start_date . " to " . $req_end_date    . "</h2>";
                $display_block .= "<div id=\"links_report_new\"><a href=\"../admin/index.php\">Back to reports index</a> | <a href=\"discounts_by_person.php\">Select a different person</a></div>";
                $display_block .= "<div id=\"text\" align=\"left\"><p>No discounts for this person</p></div>";
            }
            }
           ...
           if (mysql_num_rows($get_discounts_res)> 0)
            ...
            {
                $display_block .= "<h2 align=\"left\">Discounts for " . $fname . " " . $lname .  " <span style=\"color:#FF0000\">(inactive)</span> from " . $req_start_date . " to " . $req_end_date    . "</h2>";
                $display_block .= "<div id=\"links_report_new\"><a href=\"index_rdiscounts for this person</p></div>";
            }
...
$sql = "SELECT reservation_entry.id, band_name, start_time, misc_charge.name, reservation_transaction.comment,
            reservation_transaction.amount, reservation_transaction.updated_at
            FROM reservation_entry, reservation_transaction, misc_charge, band
            WHERE reservation_entry.id = reservation_transaction.reservation_entry_id
            AND reservation_transaction.misc_charge_id = misc_charge.id
...
<html>
    <head>
    <title>Report: discounts by person</title>
