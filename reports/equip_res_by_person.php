...
$sql = "SELECT p.fname AS first_name, p.lname AS last_name, re.id as reservation_id, eq.id as equip_id,
        eq.equip_description as equip_description, rt.updated_at as rental_date,
        rt.amount as charge_amount
        FROM person p, reservation_entry re, reservation_transaction rt, equipment eq
        WHERE p.id = re.person_id
        and re.id = rt.reservation_entry_id
        and eq.id = rt.equipment_id
        ORDER BY p.lname, p.fname,eq.id
            ";
...
?>
  ...
  <body>
  <h2>Equipment Reservations by Person <? echo $start_date ?> to <? echo $end_date?></h2>
    <div>
...
// create query
$query = "SELECT p.fname AS first_name, p.lname AS last_name, re.id as reservation_id, eq.id as equip_id,
        eq.equip_description as equip_description, rt.updated_at as rental_date,
        rt.amount as charge_amount
        FROM person p, reservation_entry re, reservation_transaction rt, equipment eq
        WHERE p.id = re.person_id
...
else
{
    ...
    while($row = mysql_fetch_array($result))
    {
       ...
       // $tot_actual_duration += $actual_minute_duration;
        ...
        $equip_desc = $row['equip_description'];
