...
$display_block .= "<h2 align=\"left\">View off-site equipment revenue</h2>";
...
$get_rev = "SELECT reservation_entry_id, fname, lname, start_time, reservation_status,
SUM(reservation_transaction.amount) AS total_payment
FROM reservation_transaction, misc_charge, reservation_entry, person
...
AND reservation_entry.person_id = person.id AND misc_charge_id IS NOT NULL
AND misc_charge_type_id = 6 AND reservation_entry.type = 'E'
AND start_time >= $utf_start_date AND end_time <= $utf_end_date
GROUP BY reservation_entry_id ORDER BY start_time, lname, fname, reservation_entry_id;";
$get_rev_res = mysql_query($get_rev) or die ("ERROR 1: " . mysql_errno() . "-" . mysql_error() );

if (mysql_num_rows($get_rev_res) == 0)
{
    $display_block .= "<h2 align=\"left\">Off-site equipment revenue for reservations with start times between " . $req_start_date . " to " . $req_end_date    . "</h2>";
    $display_block .= "<div id=\"links_report_new\"><a href=\"../admin/index.php\">Back to reports index</a> | <a href=\"offsite_equipment_revenue.php\">Select a different date range</a></div>";
...
elseif (mysql_num_rows($get_rev_res) > 0)

    {
        ...
        $display_block .= "<th>Reservation id</th><th>Person name</th><th>Reservation start time</th><th>Reservation status</th><th>Total paid</th>";
        ...
        while ($rev_info = mysql_fetch_array($get_rev_res))
            ...
            $display_block .= "<tr><td>" . $res_id . "</td><td>" . $fname . " " . $lname . "</td><td>" . utf_period_2_date($start_time) . "</td><td>" . $reservation_status . "</td><td>" . number_format(abs($total_payment), 2) . "</td></tr>";
            
            }
            $display_block .= "<tr><td colspan=\"4\"><b>Total paid for this time period</td><td><b>" . number_format(abs($grand_total_payment), 2) . "</b></td></tr>";
...
$sql = "SELECT reservation_entry_id, fname, lname, start_time, reservation_status,
SUM(reservation_transaction.amount) AS total_payment
FROM reservation_transaction, misc_charge, reservation_entry, person
WHERE reservation_transaction.misc_charge_id = misc_charge.id
AND reservation_entry.id = reservation_transaction.reservation_entry_id
...
<html>
 <head>
    <title>Report: off-site equipment revenue</title>
