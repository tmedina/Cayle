...
$display_block .= "<h2 align=\"left\">Total rentals per rental item</h2>";
...
if (mysql_num_rows($get_count_res) > 0)

    {
        ...
        while ($count_info = mysql_fetch_array($get_count_res))
        {
            ...
            $display_block .= "<div id=\"links_report_new\"><a href=\"../admin/index.php\">Back to reports index</a> | <a href=\"total_equipment_rentals.php\">Select a different item</a></div>";
...
elseif  (mysql_num_rows($get_count_res)> 0)
    {
            ...
            $inhouse_rentals = $count_info['inhouse_rentals'];
            $total_rentals = $count_info['total_rentals'];
            $grand_total_rentals += $total_rentals;
            $grand_total_offsite += $offsite_rentals;
            $grand_total_inhouse += $inhouse_rentals;

            $display_block .= "<tr><td>" . $equip_description . "</td><td>" . $offsite_rentals . "</td><td>" . $inhouse_rentals . "</td><td>" . $total_rentals . "</tr>";
           
            }

            $display_block .= "<tr><td><b>Total</b></td><td><b>" . $grand_total_offsite ."</b></td><td><b>" . $grand_total_inhouse . "</b></td><td><b>" . $grand_total_rentals . "</b></tr>";
            $display_block .= "</table><br />
            <input type=hidden name=utf_start_date value=$utf_start_date>
            <input type=hidden name=utf_end_date value=$utf_end_date>
...
<html>
 <head>
    <title>Report: total rentals per rental item</title>
