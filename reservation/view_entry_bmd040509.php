...
else
{
  $sql = "
   ...
   FROM  $tbl_entry, $tbl_room, $tbl_area, person
...
<?php
    ...
    $my_start_ds   = period_date_string_yyyymmdd ( $row['start_time'] );
    $my_end_ds     = period_date_string_yyyymmdd ( $row['end_time'] );
    ...
    if ( $my_start_d == $my_curr_d )
    {
        ...
        {
            echo "<div>";
            echo "<a href=\"check_out.php?id=$id&amp;returl=$returl\" onclick=\"return confirm('".get_vocab("confirmcheckout")."');\">".get_vocab("checkout")."</a>";
            echo "</div>";
        }
    }
?>
    ...
    <div>
    <?php
    if ( ! $series && ! isset($actual_end_time))
    {
