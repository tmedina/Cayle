 ...
 * Date         Who      What
 ***********************************************************************************
 * 02-27-2009   wilson   Added tblName, main_menu_id and sub_menu_id to url
 *                       for sort, clear sort and clear search
 *
 */
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function display_list_table_buttons($position, $listall = false) /* {{{ */
	{
		if (($but_str = $this->display_buttons($position)) === null)
			return;
		//wd if($position == 'down') echo '<hr class="'.$this->getCSSclass('hr', 'down').'" />'."\n";
		...
		echo '</tr></table>',"\n";
		//wdif($position == 'up') echo '<hr class="'.$this->getCSSclass('hr', 'up').'" />'."\n";
	} /* }}} */
	...
	function display_record_buttons($position) /* {{{ */
	{
		...
		if ($position == 'down') {
			...
			//wd echo '<hr class="',$this->getCSSclass('hr', 'down'),'" />',"\n";
		}
		echo '<table summary="navigation" class="',$this->getCSSclass('navigation', $position),'">',"\n";
		echo '<tr class="',$this->getCSSclass('navigation', $position),'">',"\n";
		echo '<td class="',$this->getCSSclass('buttons', $position),'">',"\n";
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			...
			if (! $this->fdd[$fd]['sort'] || $this->password($fd)) {
				...
				echo htmlspecialchars($this->page_name.'?'.$this->cgi['prefix']['sys'].'fm'.'=0'
						...
						.'&tblName='.$this->tb                        //wilson 02-27-2009
						.'&tblLabel='.$_REQUEST['tblLabel']          //wilson 02-27-2009
						.'&main_menu_id='.$_REQUEST['main_menu_id']   //wilson 02-27-2009
						.'&sub_menu_id='.$_REQUEST['sub_menu_id']     //wilson 02-27-2009
						.'&switch_sess='.$_REQUEST['switch_sess']);   //wilson 10-26-2006
				echo '">',$fdn,'</a></th>',"\n";
			}
		}
		echo '</tr>',"\n";
