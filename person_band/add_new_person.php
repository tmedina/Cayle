...
//Adds a newly created person to a reservation & adds a member to a band
...
if((!isset($_SESSION["logged"])) && !$_SESSION["logged"])
{
    ...
    //get person info
    $get_info = "SELECT person.id, fname, lname, address, city, state_name, zip, phone, email, person_comment, person_status FROM person, states, person_status WHERE states.id = person.state_id AND person_status.id = person.person_status_id AND person.is_active=1 AND fname='$fname' AND lname='$lname' AND phone='$phone'";
    ...
    if (mysql_num_rows($get_info_res)> 0)
    {
        ...
        while ($person_info = mysql_fetch_array($get_info_res))
        {
            ...
            $display_block .= "<td align=\"right\"><a href=\"confirm_band.php?reservation_id=$reservation_id&repeat_id=$repeat_id&return_url=$return_url&band_id=$band_id&person_id=$person_id\">Add this band & person to reservation</a></td></tr>";
            }
 ...
 if ($_POST[op] == "add_band_member")
   ...
   //display person info
    if (mysql_num_rows($get_info_res)> 0)
    {
        ...
        $get_reservation_info = "SELECT reservation_entry.id, reservation_status, start_time FROM reservation_entry, person WHERE reservation_entry.person_id = person.id AND person.id = $person_id ORDER BY start_time ASC";
        ...
        //if reservations exist display them
            if (mysql_num_rows($get_reservation_info_res)> 0)
            {
                ...
                while ($res_info = mysql_fetch_array($get_reservation_info_res))
                {
                    ...
                    //if reservation status is open display
                    else
                    {
                    $display_block .= "<tr><td>" . utf_period_2_date($start_time) . "</td></tr>";
                    }
                }
...
<html>
    ...
    <!--
    Form validation - makes sure a person is selected from the drop down menu
    -->
