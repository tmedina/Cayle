...
//This page shows band records & adds a band member to a band
...
//show selected band record
 
$band_id = $_POST[sel_id];

    //get band info
    $get_info = "SELECT * FROM band WHERE id = $_POST[sel_id]";
    ...
    if (mysql_num_rows($get_info_res)> 0)
    {
        ...
        while ($band_info = mysql_fetch_array($get_info_res))
        {
...

            {
                ...
                $display_block .= "<div id=\"member\"><h3 style=\"color:#FF0000\" align=\"left\">Activate record to add/delete/update band members & contacts</h3></div>";
            }
...
//add a band member to an existing band
 if (isset($_POST['add_band_member']))
    ...
    //get band info
    $get_info = "SELECT * FROM band WHERE is_active=1 AND id = $band_id";
    $get_info_res = mysql_query($get_info);
