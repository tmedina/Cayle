...
//This page edit a person record
...
//connect to the database & function.inc file
...
if($_GET['cmd']=="edit" || $_POST['cmd']=="edit" || isset($_POST['add_band']) || !isset($_POST['edit_new_record']))
{
   if (!isset($_POST['edit']))
   {
      $id = $_GET["id"];
      ...
      $myrow = mysql_fetch_array($result);
        ...
        //define variables
            ...
            $person_status_id_sel = stripslashes($myrow['person_status_id']);
            $person_comment = stripslashes($myrow['person_comment']);
            $user_name = stripslashes($myrow['user_name']);    
            $is_active = $myrow['is_active'];

      //display the prepopulated edit record form
      $display_block .= "<h2 align=\"left\">Edit person records - administrator view</h2>
       <div id=\"text\">";
      if ($is_active == 1)
      {
      //display the prepopulated edit record form
      ...
      {
          //display the prepopulated edit record form
      $display_block .= "<h3 style=\"color:#FF0000\" align=\"left\">This record is inactive</h3>";
      }

      $display_block .= "<h3 align=\"left\">Required *</h3>
...
if (isset($_POST['edit']))
   {
     ...
     //MySQL command to update row in person table
	  $sql = "UPDATE person SET fname='$fname', lname='$lname', address='$address', city='$city', state_id=$state_id, phone='$phone', email='$email', person_status_id=$person_status_id, person_comment='$person_comment', user_name='$user_name' WHERE id=$id";
      $result = mysql_query($sql) or die ("ERROR: " . mysql_errno() . "-" . mysql_error() );

      $get_person_info = "SELECT person.id, fname, lname, address, city, state_name, zip, phone, email, person_comment, person_status, person.is_active, user_name FROM person, states, person_status WHERE states.id = person.state_id AND person_status.id = person.person_status_id AND person.id = $id";
      ...
      if (mysql_num_rows($get_person_info_res)> 0)
        ...
        while ($person_info = mysql_fetch_array($get_person_info_res))
        {
            ...
            //if record is active and address exists do this
            ...
            $display_block .= "<a href=\"band.php\">Add band membership</a> | <a href=\"deactivate_person.php?person_id=$id\" onclick=\"return confirm('Are you sure you want to deactivate this record?')\">Deactivate</a> | <a href=\"edit.php?cmd=edit&id=$id\">Edit</a></td></tr></table></div>";
            }

            //if record is active and address doesn't exist do this
            ...
            //if record is inactive and address exists do this
            ...
            $display_block .= "<strong>User name: </strong>" . $user_name . "<br /><br />";
            $display_block .= "<strong>Comments: </strong>" . stripslashes($person_comment) . "<br /><br />";
            $display_block .= "<a href=\"activate_person.php?person_id=$id\" onclick=\"return confirm('Are you sure you want to activate this record?')\">Activate</a> | <a href=\"edit.php?cmd=edit&id=$d\">Edit</a></td></tr></table>";
            $display_block .= "<h3 style=\"color:#FF0000\">Record must be active to add band membership</h3></div> ";

            }

            //if record is inactive and address doesn't exist do this
            elseif ($is_active == 0 && $address == "")
            {
            $display_block .= "<h2 align=\"left\">Person record updated - administrator view</h2>";
            $display_block .= "<div id=\"links\"><a href=\"band.php\">View/create band records</a> | <a href=\"create_new_person.php\">Create a new person record</a> | <a href=\"index.php\">Select a different person</a></div>";
            ...
            $display_block .= "<strong>User name: </strong>" . $user_name . "<br /><br />";
            $display_block .= "<strong>Comments: </strong>" . stripslashes($person_comment) . "<br /><br />";
            $display_block .= "<a href=\"activate_person.php?person_id=$id\" onclick=\"return confirm('Are you sure you want to activate this record?')\">Activate</a> | <a href=\"edit.php?cmd=edit&id=$id\">Edit</a></td></tr></table>";
            $display_block .= "<h3 style=\"color:#FF0000\">Record must be active to add band membership</h3></div> ";
            ...
            AND band.id = band_member.band_id AND person_id = '$id' ORDER BY band_name ASC";
...
<html>
<head>
    <title>Edit person - administrator view</title>
