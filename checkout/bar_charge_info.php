...
$bc_qty = $_GET['bc_qty'];
$action = $_GET['action'];
//echo "Res: $reservation_id Ref page: $refresh_page <br/>";

// if user has selected a bar charge item, create reservation transaction
if ( $action == "add" )
{
    create_bar_charge_trans ( $reservation_id, $bc_id, $bc_qty );
}
...
<div id="bc_main">
    ...
    <div id="bc_trans">
          <?php
            ...
            //show_bar_charge_trans($reservation_id);  //moved to invoice_trans.php
          ?>
    </div>
</div>
...
//
function create_bar_charge_trans ( $reservation_id, $bc_id, $bc_qty )
{
    //echo "Res ID: $reservation_id bc_id: $bc_id bc_qty: $bc_qty";
    //exit (0);
    ...
    else
    {
        $query = "INSERT INTO reservation_transaction (reservation_entry_id, bar_charge_id, qty, amount)
                       VALUES ($reservation_id, $bc_id, $bc_qty,
...
function show_bar_charge_trans ( $reservation_id )
{
    ...
    if (mysql_num_rows($result)>0)
    {
        ...
        echo "<table border=1><tr><th>Item x Qty</th><th>Amount</th><th>Remove</th></tr>";
        while($row = mysql_fetch_array($result))
        {
            ...
            echo "<td><a href='$refresh_page?reservation_id=$reservation_id&action=remove&bc_id=$row_bc_id'>remove</a></td>";
...
?>
            ...
            <br/>Enter quantity: <input type=text name=bc_qty size=1 value=1><br/>
            <br/>
